...
#if !defined(VSERVICES_ETHERNET_TYPES_H)
#define VSERVICES_ETHERNET_TYPES_H

typedef enum vservice_ethernet_frame_error {
	VSERVICE_ETHERNET_BUFFER_OVERRUN,
	VSERVICE_ETHERNET_FRAMING_ERROR,
	VSERVICE_ETHERNET_LINK_ERROR
...
typedef struct {
} vservice_ethernet_state_t;

#define VSERVICE_ETHERNET_RESET_STATE (vservice_ethernet_state_t) { \
}

#define VSERVICE_ETHERNET_STATE_VALID(state) true

typedef struct {

	vservice_base_state_t base;

	vservice_ethernet_state_t ethernet;
} vservice_ethernet_protocol_state_t;

#define VSERVICE_ETHERNET_PROTOCOL_RESET_STATE (vservice_ethernet_protocol_state_t) {\
.base = VSERVICE_BASE_RESET_STATE,\
.ethernet = VSERVICE_ETHERNET_RESET_STATE }

#define VSERVICE_ETHERNET_IS_STATE_RESET(state) \
            ((state).base.statenum == VSERVICE_BASE__RESET)
#endif				/* ! VSERVICES_ETHERNET_TYPES_H */
